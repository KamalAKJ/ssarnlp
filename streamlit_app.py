...
def extract_header_window(lines, start_pattern, stop_patterns):
    ...
    for line in lines:
        if in_block:
            if any(re.match(p, line, re.I) for p in stop_patterns) or not line.strip():
                break
            block.append(line.strip())
        if re.match(start_pattern, line, re.I):
            in_block = True
    return block
                ...
                for s in re.split(r"[,;/]", sect):
                    if s.strip():
                        for name in add_short_forms(stat):
                            acts.append(f"{name} {s_type} {s.strip()}")
        else:
...
def extract_quranic_verses_block(text):
    ...
    for l in block_lines:
        surah_matches = re.findall(r'Surah\s*(\d+)', l, re.I)
        for surah in surah_matches:
            for vpart in re.finditer(r'verse[s]?\s*([\d,\-\– ]+)', l, re.I):
        ...
        sv_short = re.findall(r'Surah\s*(\d+)\s*[:]\s*(\d+)', l, re.I)
...
def search_legislation_section_strict(df, keywords, section):
    ...
    pattern = re.compile(rf'(s|ss|section)\s*{re.escape(section_clean)}', re.I)
...
def search_legislation_exact_subsection(df, keywords, exact_subsection):
    pattern = re.compile(rf'(s|ss|section)\s*{re.escape(exact_subsection)}', re.I)
    keywords = [normalize(k) for k in ([keywords] if isinstance(keywords,str) else keywords)]
    mask = df["Legislation referred"].apply(lambda lst:
        any(any(k in normalize(leg) and pattern.search(leg) for k in keywords) for leg in lst))
    return sorted(df.loc[mask, "Case Name"])
...
if df is not None:
    ...
    if not df.empty:
        ...
        if not cntdata.empty:
            ...
            prop = cntdata.copy()
            totals = prop.groupby("Year")["count"].transform("sum")
            prop["proportion"] = prop["count"] / totals
            plt.figure(figsize=(10,6))
            sns.lineplot(data=prop, x="Year", y="proportion", hue="Topic Groups", marker="o")
            st.pyplot(plt.gcf()); plt.clf()
